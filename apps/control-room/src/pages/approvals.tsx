          ...
          {collection.items.map((operation) => (
            <Panel key={operation.id} className="approval-card">
              ...
              <div className="approval-actions">
                <button className="button button--danger" type="button" disabled={busyRun === operation.runId} onClick={() => void decide(operation, "reject")}>Reject</button>
                <button className="button button--primary" type="button" disabled={busyRun === operation.runId} onClick={() => void decide(operation, "approve")}>{busyRun === operation.runId ? "Submitting…" : "Approve"}</button>
              </div>
            </Panel>
          ))}
