...
function managedConfig(options: {
  ...
  return [
    ...
    "[admin]",
    `bind-address = "${options.adminHost}:${options.adminPort}"`,
    "",
    "[admin.query-engine]",
    `pgsql-bind-address = "${options.queryHost}:${options.queryPort}"`,
    "",
    "[ingress]",
    `bind-address = "${options.ingressHost}:${options.ingressPort}"`,
    "",
  ].join("\n");
}
  ...
  static async start(options: {
    ...
    for (const [host, port] of [[ingressHost, ingressPort], [adminHost, adminPort], [queryHost, queryPort]] as const) {
      if (await isTcpOpen(host, port)) {
        throw new Error(`Managed Restate port is already in use (${host}:${port}); refusing to attach to an unknown local runtime`);
      }
    }

    await mkdir(options.dataDir, { recursive: true, mode: 0o700 });
    const configPath = join(options.dataDir, "blogmaatic-restate.toml");
    await writeFile(configPath, managedConfig({
      ingressHost,
      ingressPort,
      adminHost,
      adminPort,
