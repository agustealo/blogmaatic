...
README and feature documentation must show the product that actually ships. Canonical screenshots live under `docs/screenshots/` and are generated by `npm run product:screenshots` from the real runtime-backed Control Room.

The media capture must:

- start from fresh application state;
- authenticate through the real one-time Control Room launch path;
- create a real publisher connection through the canonical connection authority;
- validate live connection health;
- create a durable Publication Group and Automation through their canonical authorities;
- complete a real durable publication run before run/operations screenshots are taken;
- capture fixed-dimension PNGs with no browser-owned mock state;
- write `docs/screenshots/capture-manifest.json` with source commit and capture provenance;
- contain no secret values, bearer credentials, vault locators, private user data, or fabricated provider responses.

The canonical gallery and feature mapping are documented in [`docs/screenshots/README.md`](../screenshots/README.md). When a represented Control Room surface changes materially, regenerate and review the relevant screenshots before calling the consumer-facing release documentation current.

A docs-only commit after a valid capture does not require recapturing unchanged UI. The manifest's `sourceCommit` identifies the executable/UI source that produced the images.
